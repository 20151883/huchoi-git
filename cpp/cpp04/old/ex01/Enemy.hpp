...
class Enemy
{
	...
	public:
		Enemy(int hp, std::string const & type);//생성자를 통한 protected 멤버변수 초기화는 제일 마지막 상속클래스의 생성자가 하는걸로 결론 지었는데... 부모쪽에서 해도 괜찮다. 어짜피 자식클래스에서 다른 값으로 덮어씌울수있기때문에.. 그런데 직관적으로 만들기 위해서는 웬만하면 마지막 자식클래스의 생성자에서 초기화하는게 좋겠다.(의심하시오)
		virtual ~Enemy();
