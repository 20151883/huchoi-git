...
/*   Updated: 2021/06/28 23:27:33 by huchoi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								pthread_t *tid)
{
	...
	pthread_create(monitor, NULL, all_monitor, p_philo);
	i = 0;
...
void		restore_resources(t_philo *p_philo, pthread_t *tid)
{
	...
	free(p_philo->p_syn->eat_count_mutex);
	free(p_philo->p_syn->revision_time);
	free(p_philo->p_syn);
	free(p_philo);
}
