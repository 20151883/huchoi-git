...
namespace ft
{
	template <typename Key, typename T, class Compare = std::less<Key>, class Allocator = std::allocator< ft::pair< Key, T > > >
	class map
	{
		private:
			typedef ft::pair<Key, T> BTData;
		...
		public:
			...
			typedef typename Allocator::template rebind<BTreeNode>::other _Alnod;//*************************************
		...
		private:
			...
			BTreeNode * MakeBTreeNode()
			{
				...
				BTreeNode *nd = _nodeAllocator.allocate(1);
				// call constuct??
				nd->left = _endNode;
				nd->right = _endNode;
				nd->parent = NULL;
			...
			{
				_allocator.construct(&(bt->value), value);
				//bt->value = value;
			}
			...
			void MakeLeftSubTree(BTreeNode * main, BTreeNode * sub)
			{
				...
				{
					_allocator.destroy(&main->left->value);
					_nodeAllocator.deallocate(main->left, 1);
					//delete main->left;
				}
				main->left = sub;
				if (sub != _endNode)
			...
			void MakeRightSubTree(BTreeNode * main, BTreeNode * sub)
			{
				...
				{
					_allocator.destroy(&main->right->value);
					_nodeAllocator.deallocate(main->right, 1);
					//delete main->right;
				}
				main->right = sub;
				 if (sub != _endNode)
			...
			void MakeLeftSubTreeAvl(BTreeNode * main, BTreeNode * sub)
			{
				if (sub == _endNode){
					main->left = _endNode;
					return;
				...
				sub->root = &_RNode;
				_endNode->root = &_RNode;
				BTreeNode *temp = sub->parent;
				main->left = sub;
				sub->parent = main;
				main->parent = temp;
			}

			void MakeRightSubTreeAvl(BTreeNode * main, BTreeNode * sub)
			{
				if (sub == _endNode){
					main->right = _endNode;
					return;
			...
			BTreeNode *rightRotate(BTreeNode *y)
			{

				BTreeNode *x = y->left;
				BTreeNode *T2 = x->right;
				//std::cout << "right reach " << std::endl;
				// Perform rotation
				// MakeRightSubTreeAvl(x, y);
				// MakeLeftSubTreeAvl(y, T2);
				if (y->parent == NULL)
				{
					_RNode = x;
					//std::cout << _RNode << std::endl;
					RenewerRoot(y);
				...
				//std::cout << "right Roatre result" << std::endl;
				//preOrder(_RNode, 1);
				//std::cout << std::endl;
				// Update heights
			...
			BTreeNode *leftRotate(BTreeNode *x)
			{
				//std::cout << "left reach " << std::endl;
				BTreeNode *y = x->right;
				BTreeNode *T2 = y->left;

				// Perform rotation
				// MakeLeftSubTreeAvl(y, x);
				// MakeRightSubTreeAvl(x, T2);
				...
				//std::cout << " left Roatre result" << std::endl;
				//preOrder(_RNode, 1);
			...
			void BSTInsert(BTreeNode ** pRoot, BSTData data)
			{
				...
				nNode = MakeBTreeNode();//이때 height값을 0으로 초기화시켜주자
				SetData(nNode, data);
				...
				if(pNode != NULL)
				{
					...
					//아래에서 위로 올라가면서
					//부모 노드의 height update 해나감 <- 검증 완료
					BTreeNode *tempNode = nNode;
					tempNode = tempNode->parent;

					while (tempNode != NULL){
						//tempNode->height++;

						tempNode->height = 1 + max(height(tempNode->left), height(tempNode->right));
						tempNode = tempNode->parent;
					}
					//height update이후 다시 nNode부터 위로 올라가면서 균형 검사
					tempNode = nNode;

					// //if(_kcompare(data.first, tempNode->left->value.first) == true)
					// 	cNode = GetLeftSubTree(cNode);
					// else
					// 	cNode = GetRightSubTree(cNode);
					//data.first < tempNode->left->value.first
					//_kcompare(data.first, tempNode->left->value.first) == true
					while (tempNode != NULL){
						//std::cout << "while" << std::endl;
						int diff = tempNode->left->height - tempNode->right->height;
						if (diff > 1 && _kcompare(data.first, tempNode->left->value.first)){
							//std::cout << "left left" << std::endl;
							tempNode = rightRotate(tempNode);
							//std::cout << "------------------------" << std::endl;
							//std::cout << "RIGHT ROTATE result " << std::endl;
							//preOrder(_RNode, 1);
							//std::cout << std::endl;
							//std::cout << "endNode : " << _endNode << std::endl;
							//preOrder2(_RNode, 1);
							//std::cout << "------------------------" << std::endl;

						}
						if (diff < -1 &&  _kcompare(tempNode->right->value.first, data.first)){
							//std::cout << "right right" << std::endl;
							tempNode = leftRotate(tempNode);
							//std::cout << "------------------------" << std::endl;
							//std::cout << "LEFT ROTATE result " << std::endl;
							//preOrder(_RNode, 1);
							//std::cout << std::endl;
							//std::cout << "endNode : " << _endNode << std::endl;
							//preOrder2(_RNode, 1);
						...
						if (diff > 1 && _kcompare(tempNode->left->value.first, data.first)){
							//std::cout << "left" << std::endl;
							tempNode->left = leftRotate(tempNode->left);
							//std::cout << "right" << std::endl;
        					tempNode = rightRotate(tempNode);
							//std::cout << "------------------------" << std::endl;
							//std::cout << "LEFT ROTATE RIGHT ROTATE result " << std::endl;
							//preOrder(_RNode, 1);
							//std::cout << std::endl;
							//std::cout << "endNode : " << _endNode << std::endl;
							//preOrder2(_RNode, 1);
						...
						if (diff < -1 && _kcompare(data.first, tempNode->right->value.first)){
							//std::cout << "right left" << std::endl;
							tempNode->right = rightRotate(tempNode->right);
        					tempNode = leftRotate(tempNode);
							//std::cout << "------------------------" << std::endl;
							//std::cout << "RIGHT ROTATE LEFT ROTATE result " << std::endl;
							//preOrder(_RNode, 1);
							//std::cout << std::endl;
							//std::cout << "endNode : " << _endNode << std::endl;
							//preOrder2(_RNode, 1);
						...
						// 	//test->height++;
						// 	test->height = 1 + max(height(test->left), height(test->right));
						// 	test = test->parent;
						// }
						//ReBalance(_RNode);
						tempNode = tempNode->parent;
			...
			public:
				void preOrder(BTreeNode* node, int level) {
					if (node != _endNode) {
						// 가장 우측 노드부터 방문
						...
						for (int i = 0;i < level && node != _RNode;i++) {
							printf("     ");
						}
						printf("%d(%d)", (node->value.first), height(node));
						preOrder(node->left, level + 1);
					}
					else{
						//std::cout << "that is endNode" << std::endl;
					}
				}
				void preOrder2(BTreeNode* node, int level) {
					if (node != _endNode) {
						// 가장 우측 노드부터 방문
						preOrder2(node->right, level + 1);
						//std::cout << "_RNode  "<< _RNode <<  "endNode " << _endNode << std::endl;
						//std::cout << node->value.first << "  " << node->parent << "   " << node << "     "<< node->left << "   " << node->right << std::endl;
						preOrder2(node->left, level + 1);
					}
					else{
						std::cout << "that is endNode" << std::endl;
					}
				...
				// 	if(root != _endNode)
				// 	{
				// 		//std::cout << root->value.first << " " << root->height << std::endl;
				// 		preOrder(root->left);
				// 		preOrder(root->right);
				// 	}
				// }
			...
			BTreeNode * BSTRemove(BTreeNode ** pRoot, BSTData target)
			{
				...
				////preOrder(_RNode, 1);
				//std::cout << "target!!!!!!!!!" << target.first << std::endl;
				...
				RenewerRoot(_RNode);
				_allocator.destroy(&pVRoot->value);
				_nodeAllocator.deallocate(pVRoot ,1);
				//delete pVRoot;
				BTreeNode *ret = dNode;
				...
				BTreeNode *tempNode = dNode->parent;
				//tempNode = tempNode->parent;
				...
				//height update이후 다시 nNode부터 위로 올라가면서 균형 검사
				tempNode = dNode->parent;

				//std::cout << tempNode << std::endl;
				//preOrder(_RNode, 1);
				//std::cout <<_RNode->value.first <<"  "<<_RNode->parent <<  std::endl;
				//std::cout <<tempNode->value.first <<"  "<<tempNode->parent <<"   " << tempNode->left <<"  " <<  _endNode << "    " << tempNode->right <<   std::endl;
				//std::cout << tempNode->left->height - tempNode->right->height << std::endl;
				while (tempNode != NULL){


					int diff = tempNode->left->height - tempNode->right->height;
					if (diff > 1 && getBalance(tempNode->left) >= 0){
						//std::cout << "left left" << std::endl;
						tempNode = rightRotate(tempNode);
						// std::cout << "------------------------" << std::endl;
						// std::cout << "RIGHT ROTATE result " << std::endl;
					...
					if (diff > 1 && getBalance(tempNode->right) <= 0){
						//std::cout << "left" << std::endl;
						tempNode->left = leftRotate(tempNode->left);
						//std::cout << "right" << std::endl;
						tempNode = rightRotate(tempNode);
			...
			void _deleteAllNode(BTreeNode * bt)
			{
				...
				_allocator.destroy(&bt->value);
				_nodeAllocator.deallocate(bt, 1);
				//delete bt;
			}
		...
		public:
			...
			~map() {
				clear();
				//_allocator.destroy(&_endNode->value);
			...
			size_type max_size() const { return _nodeAllocator.max_size() / sizeof(BTreeNode); }
			...
			{
				BTreeNode *ptr = BSTRemove(&_RNode, *pos);
				//delete ptr;
				_allocator.destroy(&ptr->value);
				_nodeAllocator.deallocate(ptr, 1);
				_size--;
			}
			...
			size_type erase( const key_type& k )
			{
				...
				{
					_size--;
					_allocator.destroy(&dNode->value);
					_nodeAllocator.deallocate(dNode, 1);
					return 1;
				}
			}
			void swap( map& other )
